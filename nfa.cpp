...
nfa::nfa(int countStates, std::vector<char> alphabet, std::vector<int> finiteStates, std::vector<std::vector<std::vector<int>>> transitFunction)
{
...
bool nfa::isAccept(std::string s)
{
	...
	for (int s1 : states) {
		int ty = std::count(finalStates.begin(), finalStates.end(), s1);
		if (ty != 0)
			return true;
	}
	return false;
